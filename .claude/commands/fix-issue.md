...
1. **Reproduce** — Read relevant code, trace call chain from symptom to root cause. Reproduce in the browser (`pnpm dev`) or via a failing Vitest test.
...
2. Edge cases — boundary conditions, null/empty, Unicode/CJK, RTL text, concurrent access
3. Security — no vulnerabilities introduced (injection, XSS); API-key hygiene (keys never logged, never bundled, never sent to the wrong provider)
4. Provider-layer correctness — UI/feature code never calls a vendor SDK directly; everything goes through the one provider interface
5. Streaming & abort handling — partial chunks handled, AbortController wired through, no leaked streams or dangling promises on cancel
6. Format preservation — translation/polish must preserve Markdown/whitespace/structure of the original
7. Duplicate code — copy-paste patterns, repeated logic that should be unified
8. Dead code — unused imports, unreachable branches, orphaned functions left behind
9. Shortcuts & patches — workarounds, TODO markers, band-aids, flags to bypass broken logic
10. Lucid compliance — Zustand selectors (no store destructuring in components), Tailwind/design tokens (no hardcoded colors), file size <300 lines
...
- User-facing behavior changed? Verify in the browser (`pnpm dev`) or with a Playwright E2E test, and update docs as needed.
...
- **Version bump**: bump the `version` field in `package.json` per rule 40 (patch for a bug fix, minor for a feature).
...
- [x] `package.json` version bumped (rule 40)
{- [x] Verified in browser / Playwright (if user-facing behavior changed)}
...
```bash
git worktree add ../lucid-worktree-{N} -b fix/issue-{N}-{slug} main
```
...
git worktree remove ../lucid-worktree-{N}

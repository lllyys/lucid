...
5) **Test (Test Runner agent)**
   - Run `pnpm check:all` (lint -> test:coverage -> build).
   - If UI flows are impacted, verify in the browser — run the app (`pnpm dev`) and exercise the flow manually, or add a Playwright E2E test.

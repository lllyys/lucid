...
| WI-5 | behavioral | consent-gated migration: pure `buildSeedFromLocal` + idempotent UPSERT seed + reversible `disconnectSync` + explicit server-purge; per-server seeded flag |

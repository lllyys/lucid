...
| 11 | Polish result flashes then disappears — the keywords-change effect resets the polish op on a same-content array reference change (sync reconcile) | OPEN | high | The POLISHED result streams in, shows briefly, then resets to the placeholder. Root cause: `PolishPanel.tsx:103-113` — the "keyword change invalidates a showing result" effect compares `keywords` by **REFERENCE** (`prevKeywords.current !== keywords`) → `resetPolish()`. `keywords = usePolishKeywordsStore((s) => s.keywords)`; the sync **reconcile re-applies keywords into the store every cycle** (`src/lib/sync/reconcile.ts` — "pass 1 applies sessions/terms/keywords", `entityToKeyword`) → a **new array reference, identical content** → the effect falsely fires. Chain: polish `done` → `useAutoRecordTask` records the task → dirty → sync cycle → reconcile re-sets keywords → false reset. Since #21 defaults sync ON + every polish auto-records, it fires on ~every polish → polish effectively unusable with sync on. Latent (effect + sync both pre-existed); surfaced by enabling sync. **Fix:** compare keyword **content** (joined values / value-equality), not the array reference — a same-content re-set must NOT reset; only a real keyword change resets + re-arms. Regression test: a keywords re-set with identical values while polish is `done` must not reset. Not design-gated (pure logic). GH: #235 |

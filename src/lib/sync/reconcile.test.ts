...
const EMPTY = { sessions: [] as Session[], terms: [] as Term[], keywords: [] as Keyword[] }

describe('reconcileStores', () => {
  it('returns the current snapshot unchanged for an empty resolved set', () => {
    const cur = { sessions: [session('s1', 'A')], terms: [], keywords: [] }
  ...
  it('upserts an existing session but PRESERVES its current tasks (the entity payload has none)', () => {
    const cur = { sessions: [session('s1', 'Old', [task('t1')])], terms: [], keywords: [] }
  ...
  it('removes a tombstoned session', () => {
    const cur = { sessions: [session('s1', 'A'), session('s2', 'B')], terms: [], keywords: [] }
  ...
  it('replaces an existing task with the same id (upsert)', () => {
    const cur = { sessions: [session('s1', 'A', [task('t1', 'old')])], terms: [], keywords: [] }
  ...
  it('upserts and tombstones terms; skips malformed', () => {
    const cur = { sessions: [], terms: [{ id: 'g1', label: 'Old', createdAt: 0, updatedAt: 0, deletedAt: null } as Term], keywords: [] }
  ...
  it('upserts and tombstones keywords; skips malformed', () => {
    ...
    const cur = { sessions: [], terms: [], keywords: out.keywords }
  ...
  it('deletes a task on the envelope id even when its tombstone payload is empty (no sessionId)', () => {
    const cur = { sessions: [session('s1', 'A', [task('t1'), task('t2')])], terms: [], keywords: [] }
